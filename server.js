...
function createChecklistRoutes(options = {}) {
  const CONFIG = {
    ...
    portalUrl: options.portalUrl || process.env.PORTAL_URL || "/portal.html",
    cacheMs: Number(options.cacheMs || 5 * 60 * 1000),
    pageSizeDefault: Number(options.pageSizeDefault || 20),
    htmlPath: options.htmlPath || path.join(__dirname, "dashboard-checklist.html"),
    logoPath: options.logoPath || path.join(__dirname, "logo.png")
  };
  ...
  function decodePossiblyBrokenText(value) {
    ...
    if (/[ÃÂâ€™â€œâ€â€“]/.test(text)) {
      try {
        ...
        // Ignora correção quando não se aplicar.
      }
    }
  ...
  async function fetchSheetCsv() {
    ...
    const url = `https://docs.google.com/spreadsheets/d/${CONFIG.sheetId}/gviz/tq?tqx=out:csv&sheet=${encodedSheetName}`;
    ...
    if (!text || /^\s*</.test(text)) {
      throw new Error("Resposta do Google Sheets não retornou CSV válido.");
    }
    return text;
  }
  ...
  async function loadCsvText() {
    try {
      ...
      if (!CONFIG.fallbackCsvPath) throw error;
      return {
        source: "csv-local",
        text: fs.readFileSync(CONFIG.fallbackCsvPath, "utf8")
      };
    }
  }
  ...
  return {
    register(app) {
      app.get("/dashboard-checklist", (_req, res) => {
        res.sendFile(CONFIG.htmlPath);
      });
      ...
      app.get("/images/logo.png", (_req, res) => {
        if (fs.existsSync(CONFIG.logoPath)) {
          res.sendFile(CONFIG.logoPath);
          return;
        }
        res.status(404).send("logo.png não encontrado.");
      });

      app.get("/api/checklist/filtros", async (_req, res) => {
        const rows = await loadRows();
        res.json(buildFilterOptions(rows));
      });

      app.get("/api/checklist/resumo", async (req, res) => {
        const rows = await loadRows();
        res.json(buildResumo(rows, getFilters(req.query)));
      });

      app.get("/api/checklist/graficos", async (req, res) => {
        const rows = await loadRows();
        res.json(buildGraficos(rows, getFilters(req.query)));
      });

      app.get("/api/checklist/detalhes", async (req, res) => {
        const rows = await loadRows();
        res.json(buildDetalhes(rows, getFilters(req.query)));
      });
    }
  };
}
