...
const PRESENTEISMO_CACHE_TTL = 5 * 60 * 1000;
...
async function loadData() {
  const now = Date.now();
  if (presenteismoCache && now - presenteismoCacheTime < PRESENTEISMO_CACHE_TTL) {
  ...
  presenteismoCache = rows.map((line) => {
    const row = {};

    headers.forEach((header, i) => {
      let value = line[i] ?? "";

      if (!n(value) && filldownColumns.includes(header) && lastSeen[header] !== undefined) {
        value = lastSeen[header];
      }
    ...
    row._date = parseDate(row["DATA"]);
    row._day = row._date ? row._date.getDate() : null;
    row._abs = parseHours(row["ABSENTEISMO"]);
    row._desconto = parseMoney(row["DESCONTO"]);
  ...
  console.log("PRESENTEISMO cache atualizado:", presenteismoCache.length);
...
function applyFilters(rows, query) {
  ...
  return rows.filter((row) => {
    if (!matchesMulti(row["UNIDADE"], unidade)) return false;
    if (!matchesMulti(row["EMPRESA"], empresa)) return false;
    if (!matchesMulti(row["PLANTÃO"], plantao)) return false;
    if (!matchesMulti(row["TURNO"], turno)) return false;
    if (!matchesMulti(row["AGENTE"], agente)) return false;
    if (!matchesMulti(row["STATUS"], status)) return false;
    if (!matchesMulti(row["STATUS DE COBERTURA"], cobertura)) return false;
...
app.get("/api/presenteismo-filtros", requireAuth, async (req, res) => {
  try {
    ...
    const uniq = (field) =>
      [...new Set(rows.map((r) => n(r[field])).filter(Boolean))]
        .sort((a, b) => a.localeCompare(b, "pt-BR"));
...
app.get("/api/presenteismo-graficos", requireAuth, async (req, res) => {
  try {
    ...
    const horasPorPlantao = {};
    const registrosPorUnidade = {};
    const cobertura = {};
    const horasPorDia = {};
    const horasPorAgente = {};
    const descontoPorPlantao = {};

    for (let i = 1; i <= 31; i++) horasPorDia[i] = 0;

    periodRows.forEach((row) => {
      const plantao = n(row["PLANTÃO"]) || "SEM";
      const unidade = n(row["UNIDADE"]) || "SEM";
      const cob = n(row["STATUS DE COBERTURA"]) || "SEM";
      const agente = n(row["AGENTE"]) || "SEM";

      horasPorPlantao[plantao] = (horasPorPlantao[plantao] || 0) + Number(row._abs || 0);
      registrosPorUnidade[unidade] = (registrosPorUnidade[unidade] || 0) + 1;
      cobertura[cob] = (cobertura[cob] || 0) + 1;
      horasPorAgente[agente] = (horasPorAgente[agente] || 0) + Number(row._abs || 0);
      descontoPorPlantao[plantao] = (descontoPorPlantao[plantao] || 0) + Number(row._desconto || 0);

      if (row._day) {
        horasPorDia[row._day] = (horasPorDia[row._day] || 0) + Number(row._abs || 0);
      }
    });
    ...
    return res.json({
      horasPorPlantao: top(horasPorPlantao, 12),
      registrosPorUnidade: top(registrosPorUnidade, 12),
      cobertura: top(cobertura, 10),
      horasPorDia,
      horasPorAgente: top(horasPorAgente, 12),
      descontoPorPlantao: top(descontoPorPlantao, 12),
      comparativo: {
        atual,
        anterior,
        label: comparison.label || "base anterior",
...
app.get("/api/presenteismo-debug", requireAuth, async (req, res) => {
  try {
    const rows = await loadData();
    return res.json({
      totalLinhas: rows.length,
    ...
    console.error("Erro /api/presenteismo-debug:", error);
    return res.status(500).json({ ok: false, message: error.message });
  }
});
