...
const CONFIG = {
  ...
  sheetId: "1vUv0AJ_bASBkkzxUWOyheJSASb4VDsaRKAl40EzRKsk",
  sheetName: "Respostas",
  fallbackCsvPath: process.env.CHECKLIST_CSV_PATH || "C:\\Users\\SPXBR31640\\Downloads\\Security App - Base de Dados - Respostas.csv",
  portalUrl: process.env.PORTAL_URL || "/portal.html",
  cacheMs: 5 * 60 * 1000,
  pageSizeDefault: 20
};

const appRoot = __dirname;
const htmlPath = path.join(appRoot, "dashboard-checklist.html");
const logoPath = path.join(appRoot, "logo.png");
...
function sendFile(res, filePath, contentType) {
  fs.readFile(filePath, (error, buffer) => {
    if (error) {
      sendText(res, "Arquivo não encontrado.", 404);
      return;
    }
...
function decodePossiblyBrokenText(value) {
  ...
  if (/[ÃÂâ€™â€œâ€â€“]/.test(text)) {
    try {
      ...
      // Ignora correção de encoding quando não for aplicável.
    }
  }
